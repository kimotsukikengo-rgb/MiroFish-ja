...
def get_locale() -> str:
    if has_request_context():
        raw = request.headers.get('Accept-Language', 'ja')
        return raw if raw in _translations else 'ja'
    return getattr(_thread_local, 'locale', 'ja')


def t(key: str, **kwargs) -> str:
    locale = get_locale()
    messages = _translations.get(locale, _translations.get('ja', {}))
...
def get_language_instruction() -> str:
    locale = get_locale()
    lang_config = _languages.get(locale, _languages.get('ja', {}))
    return lang_config.get('llmInstruction', '日本語で回答してください。')
